...
export const DeliveryView = () => {
  ...
  const filteredOrders = salesOrders.filter((o) => {
    ...
    // Check if order or any item is ready/delivered
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.4rem', background: '#e2e8f0', padding: '0.2rem', borderRadius: '8px' }}>
          <button
            ...
            style={{ border: 'none' }}
          >
            <Package size={14} /> Ready for Dispatch ({salesOrders.filter((o) => o.productionStatus === 'Ready for Delivery' || o.productionStatus === 'Ready').length})
          </button>
          <button
            ...
            style={{ border: 'none' }}
          >
            <CheckCircle2 size={14} /> Delivered Orders ({salesOrders.filter((o) => o.productionStatus === 'Delivered').length})
          </button>
          <button
                    ...
                    <tr key={order.id} style={{ background: !isCleared && !isDelivered ? '#fff1f2' : 'inherit' }}>
                      ...
                      <td>
                        {isCleared ? (
                          ...
                          <span className="badge badge-rose" style={{ fontWeight: 800, fontSize: '0.78rem' }}>
                            ₹{order.balanceAmount.toLocaleString()} Pending
                          </span>
                        )}
                      </td>
                      ...
                      <td style={{ textAlign: 'center' }}>
                        {isDelivered ? (
                          ...
                          >
                            <DollarSign size={14} /> Collect ₹{order.balanceAmount.toLocaleString()}
                          </button>
                        )}
                      </td>
                    </tr>
            ...
            <form onSubmit={handleCollectPaymentSubmit} style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ background: '#fff1f2', padding: '0.85rem', borderRadius: '8px', border: '1px solid #fecdd3', fontSize: '0.84rem' }}>
                ...
                <div style={{ color: '#e11d48', fontWeight: 900, fontSize: '1rem', marginTop: '0.3rem' }}>
                  Outstanding Balance Due: ₹{collectPaymentOrder.balanceAmount.toLocaleString()}
                </div>
              </div>
